#include <errno.h>
#include <getopt.h>
#include <stdlib.h>
#include <string.h>
#include <sys/stat.h>
#include <stdio.h>
#include <unistd.h>
#include <stdint.h>

#include "workload.h"
#include "gfclient.h"
#include "steque.h"
#include "pthread.h"

#define USAGE                                                                 \
"usage:\n"                                                                    \
"  webclient [options]\n"                                                     \
"options:\n"                                                                  \
"  -s [server_addr]    Server address (Default: 0.0.0.0)\n"                   \
"  -p [server_port]    Server port (Default: 8888)\n"                         \
"  -w [workload_path]  Path to workload file (Default: workload.txt)\n"       \
"  -t [nthreads]       Number of threads (Default 1)\n"                       \
"  -n [num_requests]   Requests download per thread (Default: 1)\n"           \
"  -h                  Show this help message\n"                              \


#define BUFFER_SIZE 80

/* OPTIONS DESCRIPTOR ====================================================== */
static struct option gLongOptions[] = {
  {"server",        required_argument,      NULL,           's'},
  {"port",          required_argument,      NULL,           'p'},
  {"workload-path", required_argument,      NULL,           'w'},
  {"nthreads",      required_argument,      NULL,           't'},
  {"nrequests",     required_argument,      NULL,           'n'},
  {"help",          no_argument,            NULL,           'h'},
  {NULL,            0,                      NULL,             0}
};

static void Usage(){ fprintf(stdout, "%s", USAGE);  }
void error(const char* _message){ perror(_message); exit(0);}
typedef struct request_t{ char *path;} request_t;
char *server = "localhost";
unsigned short port = 8888;
char *workload_path = "workload.txt";


static void localPath(char *req_path, char *local_path){
  static int counter = 0;
  sprintf(local_path, "%s-%06d", &req_path[1], counter++);
}


static FILE* openFile(char *path){
  char *cur, *prev;
  FILE *ans;

  /* Make the directory if it isn't there */
  prev = path;
  while(NULL != (cur = strchr(prev+1, '/'))){
    *cur = '\0';

    if (0 > mkdir(&path[0], S_IRWXU)){
      if (errno != EEXIST){
        perror("Unable to create directory");
        exit(EXIT_FAILURE);
      }
    }

    *cur = '/';
    prev = cur;
  }

  if( NULL == (ans = fopen(&path[0], "w"))){
    perror("Unable to open file");
    exit(EXIT_FAILURE);
  }

  return ans;
}

/* Callbacks ========================================================= */
static void writecb(void* data, size_t data_len, void *arg){
  FILE *file = (FILE*) arg;

  fwrite(data, 1, data_len, file);
}

/* Struct used to track thread information,
 * Used for debug purposes and to call join()
 */
typedef struct thread_info_t {
    int         thread_num;   //ID
    pthread_t   thread_id;    //Value returned from pthread_create()
    char*       name;
    request_t*  request;
}thread_info_t;

pthread_mutex_t req_mutex = PTHREAD_MUTEX_INITIALIZER;
pthread_cond_t  req_cond = PTHREAD_COND_INITIALIZER;
steque_t        request_queue;


void cleanup(thread_info_t* _tinfo, int _nthreads){
    for(int i = 0; i < _nthreads; i++){
        free(_tinfo[i].name);
    }
    free(_tinfo);
    pthread_mutex_destroy(&req_mutex);
    pthread_cond_destroy(&req_cond);
}

/* Each worker thread call this fucntion to perform
 * client related work
 */
void perform_work(char* req_path){
    char local_path[512];
    gfcrequest_t *gfr;
    FILE *file;
    int returncode;

    localPath(req_path, local_path);
    
    file = openFile(local_path);
    
    gfr = gfc_create();
    gfc_set_server(gfr, server);
    gfc_set_path(gfr, req_path);
    gfc_set_port(gfr, port);
    gfc_set_writefunc(gfr, writecb);
    gfc_set_writearg(gfr, file);
    
    fprintf(stdout, "Requesting %s%s\n", server, req_path);
    
    if ( 0 > (returncode = gfc_perform(gfr))){
        fprintf(stdout, "gfc_perform returned an error %d\n", returncode);
        fclose(file);
        if ( 0 > unlink(local_path))
            fprintf(stderr, "unlink failed on %s\n", local_path);
    }
    else {
        fclose(file);
    }
    
    if ( gfc_get_status(gfr) != GF_OK){
        if ( 0 > unlink(local_path))
            fprintf(stderr, "unlink failed on %s\n", local_path);
    }
    gfc_cleanup(gfr);
    fprintf(stdout, "Status: %s\n", gfc_strstatus(gfc_get_status(gfr)));
    fprintf(stdout, "Received %zu of %zu bytes\n", gfc_get_bytesreceived(gfr), gfc_get_filelen(gfr));
}


/* Function call upon creating a thread
 * It dequeues a request from the queue and processes
 * request
 */
void thread_main(void* tinfo){
    thread_info_t *thread_info = tinfo;
    printf("ID: %d\n", thread_info->thread_num);
    
    while(1){
        pthread_mutex_lock(&req_mutex);

        if(steque_isempty(&request_queue)){
            pthread_mutex_unlock(&req_mutex);
            break;
        }
    
        steque_node_t* node = steque_pop(&request_queue);
        request_t* req = (request_t*) node->item;
        pthread_mutex_unlock(&req_mutex);

        perform_work(req->path);
        free(req->path); free(req); free(node);
    }
}

/* Initialiazes threads */
thread_info_t* thread_pool_init(int num_threads){
    char name[BUFFER_SIZE];
    pthread_attr_t attributes;
    pthread_attr_init(&attributes);
    thread_info_t* thread_info = calloc(num_threads, sizeof(thread_info_t));

    for (int i = 0; i < num_threads; i++) {
        memset(name, 0, BUFFER_SIZE);
        snprintf(name, BUFFER_SIZE,"thread_%d",i);
        thread_info[i].name = strdup(name);
        thread_info[i].thread_num = i;
        
        int success = pthread_create(&thread_info[i].thread_id, &attributes, (void*)&thread_main, &thread_info[i]);
        if (success != 0) error("pthread_create failed");
    }
    
    steque_init(&request_queue);
    pthread_attr_destroy(&attributes);
    
    return thread_info;
    
}

/* Helper function called by boss thread to enqueue information */
ssize_t enqueue_work(char *path){
    steque_node_t* node = calloc(1, sizeof(steque_node_t));
    request_t* request = calloc(1, sizeof(request_t));
    
    request->path = strdup(path);
    node->item = (void*)request;
    
    pthread_mutex_lock(&req_mutex);
    steque_enqueue(&request_queue, node);
    pthread_mutex_unlock(&req_mutex);
    
    return 0;
}


/* Main ========================================================= */
int main(int argc, char **argv) {
/* COMMAND LINE OPTIONS ============================================= */
    int i;
    int option_char = 0;
    int nrequests = 1;
    int nthreads = 1;
    thread_info_t* thread_info;
    

    // Parse and set command line arguments
    while ((option_char = getopt_long(argc, argv, "s:p:w:n:t:h", gLongOptions, NULL)) != -1) {
        switch (option_char) {
          case 's': // server
            server = optarg;
            break;
          case 'p': // port
            port = atoi(optarg);
            break;
          case 'w': // workload-path
            workload_path = optarg;
            break;
          case 'n': // nrequests
            nrequests = atoi(optarg);
            break;
          case 't': // nthreads
            nthreads = atoi(optarg);
            if(nthreads < 1){
                nthreads = 1;
            }
            break;
          case 'h': // help
            Usage();
            exit(0);
            break;                      
          default:
            Usage();
            exit(1);
        }
    }

    if( EXIT_SUCCESS != workload_init(workload_path)){
        fprintf(stderr, "Unable to load workload file %s.\n", workload_path);
        exit(EXIT_FAILURE);
    }

    gfc_global_init();
    thread_info = thread_pool_init(nthreads);

    /*Making the requests...*/
    for(i = 0; i < nrequests * nthreads; i++){
        char* req_path = workload_get_path();

        if(strlen(req_path) > 256){
            fprintf(stderr, "Request path exceeded maximum of 256 characters\n.");
            exit(EXIT_FAILURE);
        }

        enqueue_work(req_path);
    }
    
    pthread_cond_broadcast(&req_cond);

    for(i = 0; i < nthreads; i++){
        pthread_join(thread_info[i].thread_id, NULL);
    }
    
    cleanup(thread_info, nthreads);
    return 0;
    
}
